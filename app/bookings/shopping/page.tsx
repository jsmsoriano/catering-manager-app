    ...
    <Suspense
      fallback={
        <div className="flex h-screen items-center justify-center">
          <div className="text-lg text-zinc-600 dark:text-zinc-400">Loading shopping list...</div>
        </div>
      }
    >
...
function EventShoppingListContent() {
  ...
  if (!booking || !shoppingList) {
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="text-lg text-zinc-600 dark:text-zinc-400">Loading event shopping list...</div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">Event Shopping List</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={isLocked}
            className={`rounded-md px-4 py-2 text-sm font-medium text-white ${
              isLocked ? 'cursor-not-allowed bg-zinc-400' : 'bg-emerald-600 hover:bg-emerald-700'
            }`}
          >
            Mark Purchased
          </button>
          <button
            ...
            disabled={isLocked}
            className={`rounded-md px-4 py-2 text-sm font-medium text-white ${
              isLocked ? 'cursor-not-allowed bg-zinc-400' : 'bg-zinc-700 hover:bg-zinc-800'
            }`}
          >
            Lock List
          </button>
      ...
      <div className="mb-8 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex flex-wrap items-center gap-4 text-sm">
          ...
          <span className="text-zinc-400">|</span>
          <span className="text-zinc-600 dark:text-zinc-300">Status: {getBookingServiceStatus(booking)}</span>
          <span className="text-zinc-400">|</span>
          <span className="text-zinc-600 dark:text-zinc-300">
      ...
      <div className="mb-8 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="grid gap-3 md:grid-cols-7">
          <input
            ...
            disabled={isLocked}
            className="rounded-md border border-zinc-300 px-3 py-2 text-sm text-zinc-900 disabled:cursor-not-allowed disabled:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 dark:disabled:bg-zinc-900"
          />
          <select
          ...
          <button
            ...
            disabled={isLocked}
            className={`rounded-md px-4 py-2 text-sm font-medium text-white ${
              isLocked ? 'cursor-not-allowed bg-zinc-400' : 'bg-indigo-600 hover:bg-indigo-700'
            }`}
          >
            Add Item
          </button>
      ...
      <div className="rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 px-6 py-4 dark:border-zinc-800">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">Shopping List Items</h2>
          <button
            type="button"
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-800/50">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-zinc-600 dark:text-zinc-300">Item</th>
                  <th className="px-4 py-3 text-left font-medium text-zinc-600 dark:text-zinc-300">Category</th>
                  <th className="px-4 py-3 text-right font-medium text-zinc-600 dark:text-zinc-300">Planned</th>
                  <th className="px-4 py-3 text-right font-medium text-zinc-600 dark:text-zinc-300">Actual Qty</th>
                  <th className="px-4 py-3 text-right font-medium text-zinc-600 dark:text-zinc-300">Unit Cost</th>
                  <th className="px-4 py-3 text-right font-medium text-zinc-600 dark:text-zinc-300">Line Total</th>
                  <th className="px-4 py-3 text-center font-medium text-zinc-600 dark:text-zinc-300">Purchased</th>
                ...
                {shoppingList.items.map((item) => (
                  <tr key={item.id}>
                    <td className="px-4 py-3 text-zinc-900 dark:text-zinc-100">
                      ...
                      {item.notes && (
                        <div className="text-xs text-zinc-500 dark:text-zinc-400">{item.notes}</div>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <select
                        ...
                        className="rounded border border-zinc-300 px-2 py-1 text-xs text-zinc-900 disabled:cursor-not-allowed disabled:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 dark:disabled:bg-zinc-900"
                      >
                        <option value="food">Food</option>
                        <option value="supplies">Supplies</option>
                      </select>
                    ...
                    <td className="px-4 py-3 text-right">
                      <input
                        ...
                        className="w-24 rounded border border-zinc-300 px-2 py-1 text-right text-xs text-zinc-900 disabled:cursor-not-allowed disabled:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 dark:disabled:bg-zinc-900"
                      />
                    </td>
                    <td className="px-4 py-3 text-right">
                      <input
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        className="text-xs font-medium text-red-600 hover:text-red-700 disabled:cursor-not-allowed disabled:text-zinc-400 dark:text-red-400 dark:hover:text-red-300"
                      >
                        Remove
                      </button>
                    </td>
